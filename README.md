...
Gemini, and sends only drafts Rahul explicitly approves.
...
→ Gemini draft → Rahul reviews → Rahul approves → ConnectSafely sends
...
- Manual target-company watchlist plus Gemini company suggestions and five-factor scoring.
...
- Gemini-generated, non-template LinkedIn drafts based on Rahul's resume, job, and recipient.
...
Any API key pasted into chat must be considered exposed. Regenerate both the Google/Gemini key and
the ConnectSafely key before using this app. Never commit `.env`.

The earlier `AIza...` value is a Google API key, so this release uses it through Gemini rather than
the OpenAI API.
...
1. Use **Discover with Gemini** to auto-pick companies or add a firm manually.
...
6. Generate a unique Gemini draft, edit it, and click **Approve**.
7. Click **Send approved LinkedIn DM**. Approval does not itself send the message.
...
| `POST` | `/api/discovery/suggest` | Generate scored Gemini company suggestions |
...
| `GET` | `/api/integrations/status` | Check Gemini and LinkedIn readiness |
| `PUT` | `/api/outreach/profile` | Save Rahul's resume profile |
| `POST` | `/api/outreach/messages` | Generate a Gemini DM |
